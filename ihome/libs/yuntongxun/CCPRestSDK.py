...
class REST:
    ...
    def sendTemplateSMS(self, to,datas,tempId):
        ...
        signature = self.AccountSid + self.AccountToken + self.Batch;
        hh = hashlib.md5()
        hh.update(signature.encode('utf-8'))
        sig = hh.hexdigest().upper()
        #拼接URL
        ...
        req = requests.Request(url)
        self.setHttpHeader(req)
        req.headers['Authorization'] = auth.decode()
        #创建包体
        b=''
        for a in datas:
            b+='<data>%s</data>'%(a)
        
        body ='<?xml version="1.0" encoding="utf-8"?><SubAccount><datas>'+b+'</datas><to>%s</to><templateId>%s</templateId><appId>%s</appId></SubAccount>' %(to, tempId,self.AppId)
        req.headers['Content-Lengt'] = str(len(body))
        if self.BodyType == 'json':   
        ...
        try:
            ...
            res = requests.post(url, data=body, headers=req.headers)
            data = res.content
